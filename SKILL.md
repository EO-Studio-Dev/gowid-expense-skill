...
```bash
# 1. API 키 확인
echo "${GOWID_API_KEY:?❌ GOWID_API_KEY가 설정되지 않았습니다. Slack #dev-ops 채널의 고정 메시지를 확인하세요.}"
...
API 키가 없으면 사용자에게 안내:
> `GOWID_API_KEY`를 설정해주세요. `~/.zshrc`에 `export GOWID_API_KEY="키값"`을 추가하거나, Slack `#dev-ops` 채널의 고정 메시지를 확인하세요.
...
bash "${SKILL_DIR}/scripts/gowid.sh" whoami
```
...
모든 API 호출은 `scripts/gowid.sh`를 통해 수행. 경로:

```
~/.claude/skills/gowid-expense/scripts/gowid.sh
```
...
```bash
bash ~/.claude/skills/gowid-expense/scripts/gowid.sh my-expenses
```
...
```bash
# 용도 지정 + 제출
bash ~/.claude/skills/gowid-expense/scripts/gowid.sh submit <expenseId> <purposeId> [--memo "메모"] [--participants "id1,id2"]
```
...
```bash
bash ~/.claude/skills/gowid-expense/scripts/gowid.sh purposes
```

### 4. 팀원 목록 (참석자 선택용)

```bash
bash ~/.claude/skills/gowid-expense/scripts/gowid.sh members
```

### 5. 경비 상세 조회

```bash
bash ~/.claude/skills/gowid-expense/scripts/gowid.sh detail <expenseId>
```

### 6. 자동 분류 규칙 조회

```bash
bash ~/.claude/skills/gowid-expense/scripts/gowid.sh rules [검색어]
```
...
- 중앙 봇(Slack DM)과 병행 사용 가능 — 이미 제출된 건은 자동으로 건너뜁니다
